...
export function LoginForm() {
  ...
  // Check for OAuth conflict error from redirect
  const oauthError = searchParams.get("error");
  const resetSuccess = searchParams.get("reset") === "success";
  const conflictMessage =
    oauthError === "OAuthAccountConflict"
      ...
      <div className="w-full max-w-sm space-y-8 px-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Sign in to access your personal novel library
          </p>
        </div>
        ...
        <button
          ...
          className="inline-flex w-full items-center justify-center gap-3 rounded-lg border border-border bg-card px-4 py-3 text-sm font-medium text-card-foreground shadow-sm transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
          {GOOGLE_ICON}
          Sign in with Google
        </button>
